...
async function Wr() {
	try {
		...
		else if (n.typeId === "engine") Q("7️⃣  Register module"), await Br(e, `dpuse-engine-eu/${n.uploadGroupName}`), await zr(t);
...
async function Zr() {
	try {
		...
		let e = await Z("package.json"), t = await Z("config.json");
		switch (Q("Bump project version"), await $r("1️⃣", e), Pr(t.id).typeId) {
			case "connector":
				await Kr("2️⃣", e);
				break;
			case "context":
...
async function oi() {
	try {
		Mr("Check Dependencies"), await X("1️⃣  Check using 'npm outdated'", "npm", ["outdated"], !0), await X("2️⃣  Check using 'npm-check-updates'", "npm-check-updates", [
			"-i",
			"--dep",
			"dev,prod,peer,optional"
		]), Nr("Dependencies checked.");
	} catch (e) {
